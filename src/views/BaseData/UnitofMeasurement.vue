<template>
	<div id="UnitofMeasurement" v-loading="P_adds">
    <el-row class="container">
      <el-row>
        <el-card class="box-card" style="padding:1.5rem">
          <el-col :md="18" :lg="20" class="m-lg-10" :span="20" style="">
            <el-input style="width:26%" placeholder="请输入单位名称"></el-input>
          ...
          <el-col :span="24" class="container_padding bj">
                <table cellspacing="0" cellpadding="0" border="0" style="margin: 0 auto;border: 1px solid #F4F4F4;width:100%;margin-top: 1.5rem;">
                    <thead>
                      <tr>
                        <th colspan="1" style="text-align:center;width:7%" rowspan="1">单位名称</th>
                        <th colspan="1" style="text-align:center;width:4%" rowspan="1">单位描述</th>
        ...
        <el-dialog
          ...
          <el-form :model="form" label-width="150px">
            <el-form-item label="单位名称:" style="">
              <el-input v-model="form.unitName"></el-input>
            </el-form-item>
            <el-form-item label="单位描述:" style="">
              <textarea 
              rows="3" 
            ...
            <el-form-item class="footer_button" style="clear:both;">
              ...
              <el-button @click="UnitofMeasurement_adds()" >确认提交</el-button>
            </el-form-item>
          </el-form>
        </el-dialog>
        ...
        <el-dialog
          ...
          <el-form :model="form" label-width="150px">
            ...
            <el-form-item class="footer_button" style="clear:both;">
              ...
              <el-button @click="UnitofMeasurement_Edits()" >确认提交</el-button>
            </el-form-item>
          </el-form>
        </el-dialog>
...
import api from "../../api/api.js";
  export default {
    ...
    created() {
        let allRoles = JSON.parse(sessionStorage.getItem('newRoutes')),name = "计量单位";
    ...
    mounted() {
        ...
        this.UnitofMeasurement_info(params)  
    },
    methods: {
      ...
      UnitofMeasurement_adds(){
          ...
          this.P_adds = true;
          //新增
          api.UnitofMeasurement_adds(obj).then(res => {
              if (res.status == 200) {
                this.P_adds = false;
                  _this.$message({
                    message: "创建"+res.msg,
      ...
      UnitofMeasurement_Edits(){
          ...
          this.P_adds = true;
          //修改
          api.UnitofMeasurement_Edits(obj).then(res => {
                  ...
                  classId = attr[i];
                    ...
                    .then(_ => {
                      this.P_adds = true;
                        ...
                        api.UnitofMeasurement_Delete({id: classId}).then(res => {
                            //NProgress.done();
...
<style>
  ...
  .container_padding{
   ...
   #UnitofMeasurement .el-dialog--small{
        width: 46%;
   }
      /* 分页 */
   #UnitofMeasurement .block{
    float: none;
    padding-top: 1rem;
    text-align: center;
    display: flex;
   ...
   #UnitofMeasurement .List_button_cz{
    background: #92CC8F;
    border: 0;
    outline: 0;
    width: 1.5rem;
   ...
   #UnitofMeasurement .cell .delete{
     background: #E74C3C
   }
   #UnitofMeasurement .cell .document{
     background: #70C8CB
   }
   #UnitofMeasurement .block .el-pagination{
     padding-left: 1rem;
   }
   #UnitofMeasurement .container_padding th{
    line-height: 3rem;
    background: #fff;
    border-left: 1px solid #F4F4F4;
    border-bottom: 1px solid #F4F4F4;
   }
   #UnitofMeasurement .container_padding th:first-child{
     border-left: 0;
   }
   #UnitofMeasurement .container_padding td{
      text-align: center;
      line-height: 2.5rem;
      border-right: 1px solid #F4F4F4;
      border-bottom: 1px solid #F4F4F4;
   }
   #UnitofMeasurement .container_padding td:first-child{
     padding-left: 1px;
   }
   #UnitofMeasurement .container_padding tr:nth-child(2n+1){
     background: #F9F9F9
   }
  #UnitofMeasurement .container_padding tr:nth-child(2n+1) td{
      text-align: center;
      line-height: 2.5rem;
      border-right: 1px solid #F4F4F4;
      border-bottom: 1px solid #F4F4F4;
  ...
  #UnitofMeasurement .select_top{
    position: relative;
    width: 33.3%;
    float: left;
    height: 2.5rem;
  }
  #UnitofMeasurement .select_top span{
    display: block;
    float: left;
    line-height: 2rem;
    margin-right: 0.8rem;
  }
  #UnitofMeasurement .select_top .el-input{
    width: 75%;
    line-height: 1.8rem;
    border-radius: .2rem;
    outline: 0;
  }
  #UnitofMeasurement .box-card::-webkit-scrollbar {
      width: 0px;
  }
  #UnitofMeasurement .box-card th{
    line-height: 3rem;
    font-size: 16px;
    color: #222222;
    background: #fff;
  ...
  #UnitofMeasurement .box-card th:first-child{
    border-left: 0;
  }
  #UnitofMeasurement .box-card td{
      text-align: center;
      line-height: 2.5rem;
      border-right: 1px solid #F4F4F4;
      border-bottom: 1px solid #F4F4F4;
  }
  #UnitofMeasurement .box-card td:first-child{
    padding-left: 1px;
  }
  #UnitofMeasurement .box-card .button_div{
    padding: .5rem;
  }
  #UnitofMeasurement .el-date-editor.el-input {
      width: 100%;
  }
  #UnitofMeasurement .text th{
    border-bottom: 1px solid #d1dbe5;
    border-left: 1px solid #d1dbe5;
  }
  #UnitofMeasurement .dataS_dis_value{
    position: relative;
  }
  #UnitofMeasurement .dataS{
    position: absolute;
    width: 75%;
    background: #fff;
    height: 6rem;
  ...
  #UnitofMeasurement .el-dialog__header {
      padding: 1.5rem 1.5rem;
      background: #2C3E50;
  }
  #UnitofMeasurement .el-dialog__title{
    font-size: 18px;
    font-weight: 400;
    color: #fff;
  }
  #UnitofMeasurement .el-dialog__body{
    position: relative;
    padding: 1.5rem 1.5rem 7.5rem 1.5rem;
  }
  /* !important */
  #UnitofMeasurement .footer_button{
    position: absolute;
    margin: 0;
    width: 100%;
    height: 5.8rem;
  ...
  #UnitofMeasurement .footer_button .el-form-item__content{
      margin-left:0 !important;
      min-height: 5.8rem;
      text-align: center;
  }
  #UnitofMeasurement .footer_button .div_P{
    display: flex;
    flex-direction: row;
    justify-content: space-around;
    padding-bottom: .3rem;
  }
  #UnitofMeasurement .div_P div{
    color: #9C9C9C;
    font-size: 14px;
  }
  #UnitofMeasurement .footer_button .el-button{
    width: 6rem;
    height: 2rem;
    line-height: 0rem;
    font-size: 16px;
  ...
  #UnitofMeasurement .footer_button .el-button:hover{
    border: 1px solid #3b5168;
    background: #3b5168;
  }
  #UnitofMeasurement .header_button {
    width: 86px;
    background: #2C3E50;
    color: #fff;
    height: 34px;
  ...
  #UnitofMeasurement .header_button:hover{
    border: 1px solid transparent;
  }
  #UnitofMeasurement .header_button_a{
    width: 86px;
    background: #47A136;
    color: #fff;
    height: 34px;
  ...
  #UnitofMeasurement .header_button_a:hover{
    border: 1px solid transparent;
  }
  #UnitofMeasurement .el-card__header {
    padding: 0 20px;
    line-height: 3rem;
  }
  #UnitofMeasurement .button_list{
    float: right;
  }
  #UnitofMeasurement .button_list button{
  ...
  #UnitofMeasurement .button_list button:last-child{
  ...
  #UnitofMeasurement .uploadImages_div{
    position: absolute;
    width: 9rem;
    margin: 0;
    right: 14%;
  ...
  #UnitofMeasurement .uploadImages_div .el-form-item__content{
    margin-left:0px !important
  }
  #UnitofMeasurement .upload{
    width: 100%;
    position: relative;
    height: 8rem;
    text-align: center;
  ...
  #UnitofMeasurement .ql-editor {
      height: 5rem;
  }


@media screen and (min-width: 1200px) and (max-width: 1680px){
  #UnitofMeasurement .m-lg-1{
    width: 8.33%;
  }
  #UnitofMeasurement .m-lg-2{
    width: 16.66%;
  }
  #UnitofMeasurement .m-lg-3{
    width: 25%;
  }
  #UnitofMeasurement .m-lg-4{
    width: 33.33%;
  }
  #UnitofMeasurement .m-lg-5{
    width: 41.66%;
  }
  #UnitofMeasurement .m-lg-6{
    width: 49.98%;
  }
  #UnitofMeasurement .m-lg-7{
    width: 58.31%;
  }
  #UnitofMeasurement .m-lg-8{
    width: 66.64%;
  }
  #UnitofMeasurement .m-lg-9{
    width: 74.97%;
  }
  #UnitofMeasurement .m-lg-10{
    width: 83.30%;
  }
  #UnitofMeasurement .m-lg-11{
    width: 91.63%;
  }
  #UnitofMeasurement .m-lg-12{
    width: 100%;
  }
  #UnitofMeasurement .el-dialog--small {
      width: 46%;
