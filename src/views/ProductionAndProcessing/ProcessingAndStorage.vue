<template>
	<div id="ProcessingAndStorage" v-loading="Pass_loading">
      ...
      <el-row>
        <el-card class="box-card" style="padding:1.5rem;border:0;box-shadow:none;">
          ...
          <el-col :md="12" :lg="6" :span="6">
            <el-input class="m-lg-7" v-model="searchInput1" style="width:64%" placeholder="请输入商品名称"></el-input>
...
import api from "../../api/api.js";
  export default {
    ...
    methods: {
      ...
      ProcessingAndStorage_info(params){
        this.Pass_loading = true;
        api.ProcessingAndStorage_info(params).then(res => {
            if (res.status == 200) {
      ...
      search(searchInput){
        ...
        this.ProcessingAndStorage_info(params)
        this.flgValue = "1"
      },
                  ...
                  for(var j in attr[i])
                  {
                    if(j == "name")
                    {
                      if(attr[i].name == "idvalue")
                      {
                        ...
                        .then(_ => {
                          this.P_adds = true;
                            api.ProcessingAndStorage_Delete({id: idvalue}).then(res => {
                                if (res.status == 200) {
                                    this.P_adds = false;
                                    this.$message({
                                      message: "删除"+res.msg,
...
<style>
  ...
  .container_padding{
   ...
   #ProcessingAndStorage .block{
    ...
    #ProcessingAndStorage .List_button_cz{
    background: #92CC8F;
    border: 0;
    outline: 0;
    width: 1.5rem;
   ...
   #ProcessingAndStorage .cell .delete{
     background: #E74C3C
   }
   #ProcessingAndStorage .block .el-pagination{
     padding-left: 1rem;
   }
   #ProcessingAndStorage .container_padding th{
    line-height: 3rem;
   ...
   #ProcessingAndStorage .container_padding tr:nth-child(2n+1){
     background: #F9F9F9
   }
  #ProcessingAndStorage .container_padding tr:nth-child(2n+1) td{
      text-align: center;
      line-height: 2.5rem;
      border-right: 1px solid #F4F4F4;
      border-bottom: 1px solid #F4F4F4;
  ...
  #ProcessingAndStorage .box-card::-webkit-scrollbar {
      width: 0px;
  }
  #ProcessingAndStorage .box-card th{
    line-height: 3rem;
    font-size: 16px;
    color: #222222;
    background: #fff;
  ...
  #ProcessingAndStorage .box-card th:first-child{
    border-left: 0;
  }
  #ProcessingAndStorage .box-card td{
      text-align: center;
      line-height: 2.5rem;
      border-right: 1px solid #F4F4F4;
      border-bottom: 1px solid #F4F4F4; 
  }
  #ProcessingAndStorage .box-card td:first-child{
    padding-left: 1px;
  }
  #ProcessingAndStorage .box-card .button_div{
    padding: .5rem;
  }
  #OutboundGoods .el-date-editor.el-input {
      width: 100%;
  }
  #ProcessingAndStorage .text th{
    border-bottom: 1px solid #d1dbe5;
    border-left: 1px solid #d1dbe5;
  }
  
  #ProcessingAndStorage .el-dialog__header {
      padding: 1.5rem 1.5rem;
      background: #2C3E50;
  }
  #ProcessingAndStorage .el-dialog__title{
    font-size: 18px;
    font-weight: 400;
    color: #fff;
  }
  #ProcessingAndStorage .el-dialog__body{
    position: relative;
    padding: 1.5rem 1.5rem 7.5rem 1.5rem;
  }
  #ProcessingAndStorage .header_button {
    width: 86px;
    background: #2C3E50;
    color: #fff;
    height: 34px;
  ...
  #ProcessingAndStorage .header_button:hover{
    border: 1px solid transparent;
  }
  #ProcessingAndStorage .header_button_a{
    width: 86px;
    background: #47A136;
    color: #fff;
    height: 34px;
...
@media screen and (min-width: 1300px) and (max-width: 1680px){
  #ProcessingAndStorage .m-lg-1{
    width: 8.33% !important;
  }
  #ProcessingAndStorage .m-lg-2{
    width: 16.66% !important;
  }
  #ProcessingAndStorage .m-lg-3{
    width: 25% !important;
  }
  #ProcessingAndStorage .m-lg-4{
    width: 33.33% !important;
  }
  #ProcessingAndStorage .m-lg-5{
    width: 41.66% !important;
  }
  #ProcessingAndStorage .m-lg-6{
    width: 49.98% !important;
  }
  #ProcessingAndStorage .m-lg-7{
    width: 58.31% !important;
  }
  #ProcessingAndStorage .m-lg-8{
    width: 66.64% !important;
  }
  #ProcessingAndStorage .m-lg-9{
    width: 74.97% !important;
  }
  #ProcessingAndStorage .m-lg-10{
    width: 83.30% !important;
  }
  #ProcessingAndStorage .m-lg-11{
    width: 91.63% !important;
  }
  #ProcessingAndStorage .m-lg-12{
    width: 100% !important;
  }
  #ProcessingAndStorage .mr {
    margin-left: 0;
  }
}
</style>
